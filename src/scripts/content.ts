/**
 * The content the client's content team supplied in the four docx briefs, plus
 * the code that writes it into Payload.
 *
 * Kept separate from the scripts that call it because two of them need it:
 * `seed.ts` replaces content on a development machine, and `bootstrap.ts`
 * populates an empty production database during deploy. They differ only in
 * whether they are allowed to destroy what is already there.
 */
...
/** True when the site already has content, so bootstrap can leave it alone. */
export const hasContent = async (payload: Payload) => {
  const [services, categories] = await Promise.all([
    payload.find({ collection: "services", limit: 0 }),
    payload.find({ collection: "product-categories", limit: 0 }),
  ]);
  return services.totalDocs > 0 || categories.totalDocs > 0;
};

/** Collections this content owns, in an order safe to delete (children first). */
...
/** Creates every document and global. Deliberately deletes nothing. */
export const writeContent = async (payload: Payload) => {
